	...
	public void pushComment(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			if (osfirstid == null || osfirstid.equals("")) {
				...
				long time = circleDao.addOsFiOs(postid, userMessage.get("uid").toString(), comment,
						userMessage.get("schoolid").toString());
			...
			// 二级评论
				...
				// 添加二级评论 帖子评论量+1
				long time = circleDao.addSeOs(osfirstid, comment, userMessage.get("schoolid").toString(),
						userMessage.get("uid").toString());
				circleDao.addPostOsCount(postid);
				// 获取评论信息
				Map<String, Object> OsMessage = circleDao.queryOthOsMessageById(userMessage.get("uid").toString(),
						time);
			...
			// 二级以上评论
				...
				// 添加二级以上评论 帖子评论量+1
				long time = circleDao.addOthOs(osfirstid, comment, userMessage.get("schoolid").toString(),
						userMessage.get("uid").toString());
				circleDao.addPostOsCount(postid);
				Map<String, Object> OsMessage = circleDao.queryOthOsMessageById(userMessage.get("uid").toString(),
						time);
