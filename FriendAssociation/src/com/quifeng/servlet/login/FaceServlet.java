 ...
 * @Url 1. http：//127.0.0.1/api/user/setface 2. http：//127.0.0.1/api/user/login
 * @author 语录
 *
 */
	...
	public void signFace(HttpServletRequest request, HttpServletResponse response) throws IOException {
		...
		if (ImgeUtise.imgIsFace(mat_01)) {
			...
			if (count < 8) {
				...
				// 有8张后
				log.addUserlog(uid, System.currentTimeMillis() + "", "用户进行了注册", request.getRemoteAddr().toString(),
						"积分无变化", "普通用户");
