...
public class changecommentServlet {
	...
	public void changeComment(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			if((user.get("userzt").toString().equals("3") || user.get("userzt").toString().equals("2")) && status.equals("1")){
				jsonObject = new JSONObject();
				jsonObject.put("code", "-1");
			...
			if((!user.get("userzt").toString().equals("3") && !user.get("userzt").toString().equals("2")) && status.equals("0")){
				jsonObject = new JSONObject();
				jsonObject.put("code", "-1");
				jsonObject.put("msg", "请勿重复修改");
				writer.write(jsonObject.toJSONString());
			...
			boolean flag = false;
			if(status.equals("1")){
				if(user.get("userzt").toString().equals("1")){//同时禁止了发帖
